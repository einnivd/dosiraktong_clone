window.addEventListener("load" , ()=>{
    ...
    const swVisual = new Swiper(".sw-visual" , {
        effect: "fade",
        // loop: true 마지막 슬라이드 다음에 다시 첫 번째 슬라이드로 이동
        loop: true,
    // 슬라이드의 모션 속도를 transition 맞춘다. (1500 = 1.5s)
    speed: 1500,
    autoplay: {
      delay: 2500,
    ...
    // 2. swiper가 바뀔 때마다 실행
    // 슬라이더가 바뀌는 상태를 찾아서 우리가 적용하고자 하는 처리를 한다.
    ...
    // li태그를 클릭하면 슬라이드 기능 실행
    swVisualPgLi.forEach((item, index)=>{
        item.addEventListener("click" , function(){
            // slideToLoop()를 이용하면 원하는 페이지로 이동할 수 있다.
